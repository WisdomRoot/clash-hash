...
sponge permute = mealy step (State (Absorb 0) 0)
  where
    step :: State -> (AXI4Stream MsgBits, Bool) -> (State, AXI4Stream DigestBits)
    ...
    step (State (Permute 23) state) (_msg, _tready) = (State (Squeeze 0) (permute 23 state), idleStream)
    step (State (Permute count) state) (_msg, _tready) = (State (Permute (count + 1)) (permute count state), idleStream)
