...
module Component.SamplePolyCBD2Dev
  ( i264o12,
    i264o12Core,
    i264o24,
    i264o24Core,
...
cbd2Table bits =
  let x = pairSum (slice d1 d0 bits)
      y = pairSum (slice d3 d2 bits)
   in case (x, y) of
        (0, 0) -> 0
        (0, 1) -> 3328
        (0, 2) -> 3327
        ...
        (2, 1) -> 1
        (2, 2) -> 0
        _ -> errorX "Component.SamplePolyCBD2Dev.cbd2Table: impossible"
  where
...
stepI264o12 st (outReady, inStream) =
  case st of
    Absorb ->
      if tvalid inStream
        then
          let initState = absorb33Normal (tdata inStream)
