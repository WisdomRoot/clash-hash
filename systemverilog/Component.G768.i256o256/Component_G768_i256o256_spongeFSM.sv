/* AUTOMATICALLY GENERATED SYSTEMVERILOG-2005 SOURCE CODE.
** GENERATED BY CLASH 1.8.2. DO NOT MODIFY.
*/
`default_nettype none
`timescale 100fs/100fs
module Component_G768_i256o256_spongeFSM
    ( // Inputs
      input wire logic c$bindCsr  // clock
    , input wire logic c$bindCsr_0  // reset
    , input wire logic c$bindCsr_1  // enable
    , input wire Component_G_768_I256_O256_types::Tuple3_0 eta2

      // Outputs
    , output Component_G_768_I256_O256_types::Tuple2_4 result
    );
  Component_G_768_I256_O256_types::Tuple2_3 c$complementAt1_tupIn;
  Component_G_768_I256_O256_types::Tuple2_3 result_0;
  Component_G_768_I256_O256_types::Tuple2_3 c$complementAt1_tupIn_case_alt;
  logic [1599:0] state;
  Component_G_768_I256_O256_types::State ds1 = {{2'b00,2'd0,5'bxxxxx}
,1600'};
  Component_G_768_I256_O256_types::Tuple2 c$ds_case_alt;
  logic [255:0] result_1;
  logic [255:0] c$case_alt;
  logic [1599:0] c$app_arg;
  Component_G_768_I256_O256_types::State c$ds_app_arg;
  Component_G_768_I256_O256_types::State c$ds_case_alt_0;
  logic isLast;
  logic [0:0] counter;
  Component_G_768_I256_O256_types::Tuple2 c$ds_case_alt_1;
  logic tready;
  logic [1:0] seenTLAST;
  logic [4:0] counter_0;
  Component_G_768_I256_O256_types::Tuple2 result_2;
  Component_G_768_I256_O256_types::Tuple2 c$ds_case_alt_2;
  Component_G_768_I256_O256_types::Tuple2 c$ds_case_alt_3;
  logic [1599:0] c$ds_app_arg_0;
  logic [1599:0] result_3;
  logic [1599:0] c$app_arg_0;
  logic [1599:0] result_4;
  logic [1599:0] c$app_arg_1;
  logic [1599:0] result_5;
  logic signed [63:0] c$app_arg_2;
  logic [1599:0] c$app_arg_3;
  logic [1599:0] result_6;
  logic signed [63:0] c$app_arg_4;
  logic [1599:0] c$app_arg_5;
  logic [1599:0] result_7;
  logic signed [63:0] c$app_arg_6;
  logic [1599:0] c$app_arg_7;
  logic [10:0] c$complementAt1_arg0;
  logic [10:0] c$complementAt1_arg0_case_alt;
  logic [10:0] c$complementAt1_arg0_case_alt_0;
  logic c$complementAt1_arg0_case_scrut;
  logic [10:0] c$complementAt1_arg0_0;
  logic [10:0] c$complementAt1_arg0_case_alt_1;
  Component_G_768_I256_O256_types::Tuple2 result_8;
  Component_G_768_I256_O256_types::Tuple2 c$ds_case_alt_4;
  logic [1599:0] result_9;
  logic [1599:0] c$case_alt_0;
  logic [1599:0] c$case_alt_1;
  logic [1599:0] c$case_alt_2;
  logic [1599:0] c$case_alt_3;
  logic [1599:0] c$app_arg_8;
  logic c$ds_case_scrut;
  logic c$ds_case_scrut_0;
  logic flush;
  logic c$ds_app_arg_1;
  logic [1:0] counter_1;
  Component_G_768_I256_O256_types::AXI4Stream \input ;
  logic [8:0] c$ds_case_alt_selection_1;
  logic [10:0] c$i_18;
  logic [255:0] c$bv;

  assign result = c$ds_case_alt.Tuple2_sel1;

  assign c$complementAt1_tupIn = c$ds_case_scrut_0 ? {11'd1
                                                     ,state} : result_0;

  assign result_0 = c$ds_app_arg_1 ? {11'd256
                                     ,result_9} : c$complementAt1_tupIn_case_alt;

  assign c$complementAt1_tupIn_case_alt = c$complementAt1_arg0_case_scrut ? {11'd513
                                                                            ,result_9} : {11'd1,result_9};

  assign state = ds1.State_sel1;

  // register begin
  always_ff @(posedge c$bindCsr or  posedge  c$bindCsr_0) begin : ds1_register
    if ( c$bindCsr_0) begin
      ds1 <= {{2'b00,2'd0,5'bxxxxx}
  ,1600'};
    end else  if (c$bindCsr_1)  begin
      ds1 <= c$ds_case_alt.Tuple2_sel0;
    end
  end
  // register end

  assign c$ds_case_alt_selection_1 = ds1.State_sel0;

  always_comb begin
    case(c$ds_case_alt_selection_1[8:7])
      2'b00 : c$ds_case_alt = result_2;
      2'b01 : c$ds_case_alt = c$ds_case_alt_1;
      default : c$ds_case_alt = {c$ds_app_arg
                                ,{{result_1,1'b1,isLast},1'b0}};
    endcase
  end

  assign result_1 = (counter == 1'd0) ? (c$app_arg[255 : 0]) : c$case_alt;

  assign c$case_alt = (counter == 1'd1) ? (c$app_arg[511 : 256]) : ({256 {1'bx}});

  assign c$app_arg = state;

  assign c$ds_app_arg = (~ tready) ? ds1 : c$ds_case_alt_0;

  assign c$ds_case_alt_0 = isLast ? {{2'b00,2'd0,5'bxxxxx}
                                    ,1600'} : {{2'b10,counter + 1'd1,6'bxxxxxx}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               ,state};

  assign isLast = counter == 1'd1;

  assign counter = ds1[1606:1606];

  Component_G768_i256o256_sponge20 Component_G768_i256o256_sponge20_c$ds_case_alt_1
    ( .result (c$ds_case_alt_1)
    , .eta3 (counter_0)
    , .eta4 (seenTLAST)
    , .eta5 (state)
    , .eta6 (tready) );

  assign tready = eta2.Tuple3_0_sel1;

  assign seenTLAST = ds1[1601:1600];

  assign counter_0 = ds1[1606:1602];

  assign result_2 = c$ds_case_scrut_0 ? {{{2'b01,5'd0,2'd0}
                                         ,result_5}
                                        ,{{256'
                                          ,1'b0
                                          ,1'b0}
                                         ,1'b0}} : c$ds_case_alt_2;

  assign c$ds_case_alt_2 = (~ \input .AXI4Stream_sel1) ? {ds1
                                                         ,{{256'
                                                           ,1'b0
                                                           ,1'b0}
                                                          ,1'b1}} : c$ds_case_alt_3;

  assign c$ds_case_alt_3 = (\input .AXI4Stream_sel2 & c$ds_case_scrut) ? {{{2'b01,5'd0,2'd0}
                                                                          ,c$ds_app_arg_0}
                                                                         ,{{256'
                                                                           ,1'b0
                                                                           ,1'b0}
                                                                          ,1'b0}} : result_8;

  assign c$ds_app_arg_0 = c$ds_app_arg_1 ? result_3 : result_5;

  // replaceBit start
  always_comb begin
    result_3 = c$app_arg_0;
    result_3[64'sd575] = (~ (c$app_arg_0[64'sd575]));
  end
  // replaceBit end

  assign c$app_arg_0 = result_4;

  // replaceBit start
  always_comb begin
    result_4 = c$app_arg_1;
    result_4[64'sd266] = (~ (c$app_arg_1[64'sd266]));
  end
  // replaceBit end

  assign c$app_arg_1 = result_5;

  // replaceBit start
  always_comb begin
    result_5 = c$app_arg_3;
    result_5[c$app_arg_2] = (~ (c$app_arg_3[c$app_arg_2]));
  end
  // replaceBit end

  assign c$app_arg_2 = $unsigned({{(64-11) {1'b0}},c$complementAt1_arg0_0});

  assign c$app_arg_3 = result_6;

  // replaceBit start
  always_comb begin
    result_6 = c$app_arg_5;
    result_6[c$app_arg_4] = (~ (c$app_arg_5[c$app_arg_4]));
  end
  // replaceBit end

  assign c$app_arg_4 = $unsigned({{(64-11) {1'b0}},c$complementAt1_arg0});

  assign c$app_arg_5 = result_7;

  // replaceBit start
  always_comb begin
    result_7 = c$app_arg_7;
    result_7[c$app_arg_6] = (~ (c$app_arg_7[c$app_arg_6]));
  end
  // replaceBit end

  assign c$i_18 = c$complementAt1_tupIn.Tuple2_3_sel0;

  assign c$app_arg_6 = $unsigned({{(64-11) {1'b0}},c$i_18});

  assign c$app_arg_7 = c$complementAt1_tupIn.Tuple2_3_sel1;

  assign c$complementAt1_arg0 = c$ds_case_scrut_0 ? 11'd2 : c$complementAt1_arg0_case_alt;

  assign c$complementAt1_arg0_case_alt = c$ds_app_arg_1 ? 11'd257 : c$complementAt1_arg0_case_alt_0;

  assign c$complementAt1_arg0_case_alt_0 = c$complementAt1_arg0_case_scrut ? 11'd514 : 11'd2;

  assign c$complementAt1_arg0_case_scrut = counter_1 == 2'd1;

  assign c$complementAt1_arg0_0 = c$ds_case_scrut_0 ? 11'd575 : c$complementAt1_arg0_case_alt_1;

  assign c$complementAt1_arg0_case_alt_1 = c$ds_app_arg_1 ? 11'd265 : 11'd575;

  assign result_8 = \input .AXI4Stream_sel2 ? {{{2'b01,5'd0,2'd1}
                                               ,result_9}
                                              ,{{256'
                                                ,1'b0
                                                ,1'b0}
                                               ,1'b0}} : c$ds_case_alt_4;

  assign c$ds_case_alt_4 = c$ds_case_scrut ? {{{2'b00,counter_1 + 2'd1,5'bxxxxx}
                                              ,result_9}
                                             ,{{256'
                                               ,1'b0
                                               ,1'b0}
                                              ,1'b1}} : {{{2'b01,5'd0,2'd2},result_9}
                                                        ,{{256'
                                                          ,1'b0
                                                          ,1'b0}
                                                         ,1'b0}};

  assign result_9 = (counter_1 == 2'd0) ? c$case_alt_0 : c$case_alt_1;

  // setSlice begin
  always_comb begin
    c$case_alt_0 = c$app_arg_8;
    c$case_alt_0[255 : 0] = ((c$app_arg_8[255 : 0]) ^ \input .AXI4Stream_sel0);
  end
  // setSlice end

  assign c$case_alt_1 = (counter_1 == 2'd1) ? c$case_alt_2 : c$case_alt_3;

  // setSlice begin
  always_comb begin
    c$case_alt_2 = c$app_arg_8;
    c$case_alt_2[511 : 256] = ((c$app_arg_8[511 : 256]) ^ \input .AXI4Stream_sel0);
  end
  // setSlice end

  assign c$bv = (\input .AXI4Stream_sel0);

  // setSlice begin
  always_comb begin
    c$case_alt_3 = c$app_arg_8;
    c$case_alt_3[575 : 512] = ((c$app_arg_8[575 : 512]) ^ (c$bv[63 : 0]));
  end
  // setSlice end

  assign c$app_arg_8 = state;

  assign c$ds_case_scrut = counter_1 < 2'd2;

  assign c$ds_case_scrut_0 = flush & c$ds_app_arg_1;

  assign flush = eta2.Tuple3_0_sel2;

  assign c$ds_app_arg_1 = counter_1 == 2'd0;

  assign counter_1 = ds1[1606:1605];

  assign \input  = eta2.Tuple3_0_sel0;


endmodule

