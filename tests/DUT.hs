...
driveMessage dut inputMsg =
  let (digestSig, doneSig, _, _, _, _, _, _, _, _, _) = driveMessageDebug dut inputMsg
...
driveMessageDebug dut inputMsg = (actualDigest, allBeatsCollected, sReady, mValid, mData, mLast, beatNum, sValid, sData, sLast, mReady)
  where
