...
module Test.NonPipelined.SHAKE128B (spec) where

import Data.Foldable (for_)
import Prelude
import Test.Hspec
import Test.QuickCheck
import Test.TestHarness.SHAKE128B
import Test.TestHarness.SHAKESamples qualified as Samples

spec :: Spec
spec = describe "NonPipelined SHAKE-128B (Byte-Stream) Tests" $ do
  ...
  describe "QuickCheck property tests" $
    it "correctly handles random test cases" $
      withMaxSuccess 10 $
        forAll Samples.shake128Gen runTest
