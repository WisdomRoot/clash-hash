...
runReference zetas input =
  P.map P.toInteger
    ...
    $ Ref.ntt
        qInt
        (V.fromList (P.map normalizeInt zetas))
        (V.fromList (P.map normalizeInt input))
  where
    normalizeInt :: P.Integer -> P.Int
    normalizeInt x =
      P.fromInteger (x `P.mod` q)
...
spec :: Spec
spec =
  describe "Component.NTT full 256-point transform" P.$ do
