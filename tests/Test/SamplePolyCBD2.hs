...
instance Arbitrary SamplePolyCBD2Input where
  arbitrary = do
    ...
    b <- arbitrary
    pure (SamplePolyCBD2Input seed b)
...
spec = describe "SamplePolyCBD2" $ do
  ...
  it "matches kyber-py reference (seed=[0..31], b=0x01)" $ do
    let expected = samplePolyCBDReference Eta2 seed0 byte0
        actual = runSamplePolyCBDHardware SamplePolyCBD2.topEntity seed0 byte0
  ...
  it "matches kyber-py reference (all zeros)" $ do
    let seed = BS.replicate 32 0
        ...
        actual = runSamplePolyCBDHardware SamplePolyCBD2.topEntity seed b
    actual `shouldBe` expected
...
prop_samplePolyCBD2 (SamplePolyCBD2Input s b) =
  let expected = samplePolyCBDReference Eta2 s b
      actual = runSamplePolyCBDHardware SamplePolyCBD2.topEntity s b
   in actual == expected
