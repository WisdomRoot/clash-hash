...
def run_cmd(cmd, label, timeout=36000):
...
def collect_stack_build_artifacts() -> list[Path]:
    patterns = [
        ".stack-work/dist/*/ghc-*/build/libHSml-dsa*.a",
        ".stack-work/dist/*/ghc-*/build/libHSml-dsa*.so",
        ".stack-work/dist/*/ghc-*/build/libHSml-dsa*.dylib",
        ".stack-work/dist/*/ghc-*/package.conf.inplace/ml-dsa-*.conf",
    ]
    paths: list[Path] = []
...
def bench(target_label: str):
    ...
    run_timed_stage("build", lambda: run_cmd(["stack", "build", "ml-dsa:lib"], "stack build ml-dsa:lib"))
